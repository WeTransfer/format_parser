...
class FormatParser::Decoder
  ...
  class Atom < Struct.new(:type, :position, :size, :fields, :children)
    ...
    # Find and the return the first descendent (using breadth-first search) of a given type.
    def find_shallowest_descendent(types, depth = 1)
      return if children.empty?

      if (child = children.find { |child| types.include?(child.type) })
        return [child, depth] if child
      end
    end
    ...
    def select_descendents(types)
      ...
      children.filter_map do |child|
        ...
        return types.include?(child.type) ? [child] + descendents : descendents
      end.flatten
    end
  end
  ...
  def build_atom_tree(max_read, io = nil)
    ...
    atoms = []
    atom_end = @buf.pos + max_read
    loop do
      break if @buf.pos >= atom_end
      atom = parse_atom
      break unless atom
      atoms << atom
    end
